...
public class RickshawAIControl : MonoBehaviour
{
    ...
    [SerializeField] private Transform[] m_Targets;                                              // 'target' the target object to aim for.
    ...
    {
        currentTarget += 1;
        if (currentTarget == m_Targets.Length)
        {
            currentTarget = 0;
        }
        m_Target = m_Targets [currentTarget];
    }
    ...
    private void OnCollisionStay(Collision col)
    {
        ...
        {
            var otherAI = col.rigidbody.GetComponent<RickshawAIControl>();
            if (otherAI != null)
            {
    ...
    {
        m_Target = target;
        if (transform.InverseTransformPoint(target.position).magnitude >= m_ReachTargetThreshold)
        {
            m_Driving = true;
        }
    }
